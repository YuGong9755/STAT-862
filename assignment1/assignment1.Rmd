...
for (i in 1:6){
        ...
        var[i]<-sum(predict_test$se.fit)/100
        mse[i]<-bias[i]+var[i]
}

plot(id,bias,type='l',col='red',ylim=c(0,10),ylab="bias&var&mse")
lines(id,var,type='l',col='blue',ylim=c(0,10))
lines(id,mse,type='l',col='black',ylim=c(0,10))
```
...
### (a)
```{r}
fit<-lm(lpsa~lcavol+lweight+age+lbph+svi+lcp+gleason+pgg45,data=train)
predict_test<-predict.lm(fit,test,interval = "none")
predict_test
MSE<-sum((predict_test-test$lpsa)^2)/length(predict_test)
MSE
```
### (b)
```{r}
...
for (i in 1:length(m)) {
        ...
        result[i]<-sum((knn_predicted$pred-test_n$lpsa)^2)
}
result
```
